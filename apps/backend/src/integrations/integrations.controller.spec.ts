...
describe('IntegrationsController', () => {
  ...
  describe('connect', () => {
    it('should call integrationService.connectIntegration with correct parameters', async () => {
      ...
      const mockResult = { id: 'account-1', userId: 'user-123', platform: Platform.Codeforces, handle: 'testuser' };

      vi.spyOn(service, 'connectIntegration').mockResolvedValue(mockResult as any);

      const result = await controller.connect(req as any, dto);

      expect(service.connectIntegration).toHaveBeenCalledWith('user-123', Platform.Codeforces, 'testuser');
  ...
  describe('sync', () => {
    it('should call integrationService.triggerSync with correct parameters', async () => {
      ...
      const mockResult = { id: 'int-456', status: 'Connected', lastSync: new Date() };
